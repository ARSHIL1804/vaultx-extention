...
import { addCoins } from "@/packages/@cosmjs/amino";
import { toHex } from "@/packages/@cosmjs/encoding";
import { Uint53 } from "@/packages/@cosmjs/math";
import { CometClient, connectComet, HttpEndpoint, toRfc3339WithNanoseconds } from "@/packages/@cosmjs/tendermint-rpc";
import { assert, sleep } from "@/packages/@cosmjs/utils";
...
export interface IndexedTx {
   ...
   *
   * ```js
   * import { sha256 } from "@/packages/@cosmjs/crypto";
   * import { toHex } from "@/packages/@cosmjs/encoding";
   *
   * const transactionId = toHex(sha256(indexTx.tx)).toUpperCase();
   * ```
   *
   * Use `decodeTxRaw` from @/packages/@cosmjs/proto-signing to decode this.
   */
